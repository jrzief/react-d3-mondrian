  ...
  let mondrian = useMemo(() => {
    const generateMondrian = ({ value, colorRatios, depth = 0 }) => {
      ...
      return {
        value,
        color: createColor({
          //(colorRatios), //({ red: 0.3, blue: 0.2, yellow: 0.6, black: 0.4 }),
          redRatio,
          yellowRatio,
          blueRatio,
          blackRatio
    ...
    <div className="App">
      <header>
        ...
        <div>
          ...
          <Range name="yellow" value={yellowRatio} onChange={setYellowRatio} />
          <Range name="blue" value={blueRatio} onChange={setBlueRatio} />
          <Range name="black" value={blackRatio} onChange={setBlackRatio} />
        </div>
      </header>
      <svg width="100%" height="100%" style={{ border: '1px solid red' }}>
        <Mondrian x={100} y={10} width={800} height={400} data={mondrian} />
      </svg>
    </div>
  );
}
